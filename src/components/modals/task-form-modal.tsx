...
// Removi o ScrollArea dos imports pois vamos usar rolagem nativa
import {
    Select,
    SelectContent,
    SelectItem,
...
export function TaskFormModal({
        ...
        if (isOpen) {
            ...
            if (initialData) {
                form.reset({
                    ...
                    // Mapeamento do Banco para o Form
    ...
    const handleSubmit = async (values: TaskFormValues) => {
        ...
        const hasAddress = street || city || (latitude && longitude);
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            {/* max-h-[90vh] limita a altura e flex-col organiza Header, Corpo e Footer */}
            <DialogContent className="max-w-3xl max-h-[90vh] flex flex-col p-0 gap-0"> 
                ...
                {/* CORPO COM SCROLL VERTICAL NATIVO */}
                    ...
                    <Form {...form}>
                        <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
                            ...
                            <Tabs defaultValue="info" className="w-full">
                                ...
                                {/* TAB 2: LOCALIZAÇÃO */}
                                <TabsContent value="address" className="space-y-5 animate-in fade-in">
                                    <div className="space-y-4">
                                        ...
                                        <div className="p-3 bg-blue-50/50 border border-blue-100 rounded-lg text-xs text-blue-700 mb-4">
                                            Digite o CEP ou Endereço e clique em "Buscar no Mapa" para preencher as coordenadas automaticamente.
                                        </div>
                                            ...
                                            <FormField control={form.control} name="cep" render={({ field }) => (
                                                <FormItem>
                                                    ...
                                                    <div className="relative">
                                                        <FormControl><Input placeholder="00000-000" maxLength={9} {...field} onBlur={handleCepSearch} /></FormControl>
                                        ...
                                        <div className="grid grid-cols-[100px_1fr] gap-4">
                                            <FormField control={form.control} name="number" render={({ field }) => (
                                                <FormItem>
                                                    ...
                                                    <FormControl><Input id="address-number" placeholder="Nº" {...field} /></FormControl>
                                                </FormItem>
                                            )} />
                                        ...
                                        <div className="pt-4 border-t border-slate-100 mt-4">
                                            <div className="flex justify-between items-center mb-3">
                                                <Label className="text-xs text-slate-500 uppercase font-bold flex items-center gap-1">
                                                    <Globe className="h-3 w-3" /> Geolocalização (GPS)
                                                </Label>
                                                <Button type="button" size="sm" variant="outline" onClick={handleGeocode} disabled={isGeocoding} className="h-7 text-xs">
                                                    {isGeocoding ? <Loader2 className="h-3 w-3 animate-spin mr-1" /> : <Search className="h-3 w-3 mr-1" />}
                                                    Buscar no Mapa
                                                </Button>
                                            </div>

                                            {/* Aviso de UX */}
                                            {form.watch('street') && !form.watch('latitude') && (
                                                <div className="mb-3 p-2 bg-yellow-50 text-yellow-700 text-xs rounded border border-yellow-200 flex items-center gap-2">
                                                    <AlertTriangle size={14} className="shrink-0" />
                                                    <span>Necessário clicar em "Buscar no Mapa" para calcular rotas.</span>
                                                </div>
                                            )}
                                            ...
                                            <div className="grid grid-cols-2 gap-4">
                                                <FormField control={form.control} name="latitude" render={({ field }) => (
                                                    <FormItem>
                                                        <FormLabel className="text-xs text-slate-500">Latitude</FormLabel>
                                                        <FormControl>
                                                            <Input 
                                                                placeholder="Latitude" 
                                                                {...field} 
                                                                readOnly 
                                                                className="bg-slate-50 border-slate-200 text-xs font-mono h-9" 
                                                            />
                                                        </FormControl>
                                                    </FormItem>
                                                )} />
                                                <FormField control={form.control} name="longitude" render={({ field }) => (
                                                    <FormItem>
                                                        <FormLabel className="text-xs text-slate-500">Longitude</FormLabel>
                                                        <FormControl>
                                                            <Input 
                                                ...
                                                <div className="grid grid-cols-5 gap-2">
                                                    {initialData.taskImages.map(img => !removedImageIds.includes(img.id) && (
                                                        <div key={img.id} className="relative aspect-square rounded overflow-hidden group border bg-white">
                                                            <img src={img.url} className="w-full h-full object-cover" alt="Task attachment" />
