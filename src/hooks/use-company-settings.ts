  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!companyId || companyId.trim() === "") {
        console.warn(
          "[useCompanySettings] Tentativa de buscar sem companyId válido",
        );
        return null; // Retorna null em vez de fazer requisição
      }

      console.log(
        `🔍 [useCompanySettings] Buscando dados para empresa ${companyId}`,
      );
  ...
  useEffect(() => {
    const handleCompanyUpdated = async (event: any) => {
      ...
      if (event.detail?.companyId === companyId) {
        console.log(
          `🔄 [useCompanySettings] Evento 'companyUpdated' detectado para ${companyId}`,
        );
    ...
    window.addEventListener(
      "companyUpdated",
      handleCompanyUpdated as EventListener,
    );
