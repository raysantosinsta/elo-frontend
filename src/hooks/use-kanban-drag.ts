...
interface UseKanbanDragProps<T extends BaseItem> {
  ...
  idField: keyof T;
  moveCallback: (itemId: string, newColumnId: string, responsibleId?: string, type?: 'user' | 'supplier') => Promise<any>; // Mudado para any para capturar resposta
...
export function useKanbanDrag<T extends BaseItem>({
  ...
  const executeMove = async (
    ...
    const item = items.find((i) => i.id === itemId);
    if (!item) {
      console.error('[useKanbanDrag] executeMove: Item não encontrado:', itemId);
      return;
    }
    ...
    try {
      ...
      // Executa o movimento
      const response = await moveCallback(itemId, targetColumnId, responsibleId, type);
      ...
      if (error.code === 'ECONNABORTED' || error.message?.includes('timeout') || error.name === 'AbortError' || error.code === 'ERR_CANCELED') {
        console.log('[useKanbanDrag] Timeout detectado, mantendo atualização otimista');
      ...
      if (error.response?.status === 500) {
        console.log('[useKanbanDrag] Erro 500 detectado - pode ter sido sucesso no backend');
