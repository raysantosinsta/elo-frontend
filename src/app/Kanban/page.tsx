...
//   // Criamos os headers base de forma segura
//   const headers: Record<string, string> = {
//     'Content-Type': 'application/json',
//     // Copiamos headers existentes apenas se existirem e forem objeto simples
//     ...(typeof options.headers === 'object' && options.headers !== null
//       ? // Aqui fazemos type guard para extrair apenas as entries que são string: string
//         Object.fromEntries(
...
//     ...options,
//     headers, // Agora headers é Record<string, string> → 100% seguro
//   });
...
//   const authFetchWithFiles = async (url: string, formData: FormData, method: string = 'POST') => {
//     const token = getAuthToken();
...
//       const response = await authFetchWithFiles(`${API_BASE}/tasks`, formData, "POST");
...
//       const response = await authFetchWithFiles(`${API_BASE}/tasks/${editingTask.id}`, formData, "PUT");
...
//   // Se não tiver usuário autenticado, não renderiza nada (será redirecionado)
//   if (!user) {
//     return null;
//   }
...
//             <h1 className="text-2xl font-bold">KANBAN DE DESENVOLVIMENTO DE PRODUTOS - {user.company?.name}</h1>
...
export default function ProductKanban() {
  ...
  useEffect(() => {
    // 🔥 CORREÇÃO: Redirecionar apenas se não estiver autenticado E não estiver carregando
    if (!authLoading && !user) {
      router.push('/login');
      return;
    }
  ...
  // 🔥 CORREÇÃO: Melhorar o loading state
  if (authLoading) {
    return (
      <div className="flex h-screen items-center justify-center">
  ...
  // 🔥 CORREÇÃO: Se não tiver usuário autenticado, não renderiza nada (será redirecionado)
  if (!user) {
    return null;
  }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <Dialog open={isTaskModal} onOpenChange={setIsTaskModal}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            {/* ... (conteúdo do modal de criar tarefa permanece igual) ... */}
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isEditTaskModal} onOpenChange={setIsEditTaskModal}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          {editingTask && (
            <div className="space-y-4">
              {/* ... (conteúdo do modal de editar tarefa permanece igual) ... */}
            </div>
          )}
        </DialogContent>
      </Dialog>
