...
export default function ProductKanban() {
  ...
  const resetTaskForm = () => {
    ...
    setTaskAssignedTo(""); setTaskColumn(""); setTaskPriority("1");
    setTaskFinalComment(""); 
    setTaskImages([]); setTaskAudios([]); setTaskVideos([]); setAudioBlob(null); setRecordingTime(0);
    setTaskZip(""); setTaskStreet(""); setTaskNumber(""); setTaskNeighborhood(""); setTaskCity(""); setTaskState(""); setTaskComplement("");
    setTaskLatitude(""); setTaskLongitude("");
  };
  ...
  const resetEditTaskForm = () => {
    ...
    setEditTaskZip(""); setEditTaskStreet(""); setEditTaskNumber(""); setEditTaskNeighborhood(""); setEditTaskCity(""); setEditTaskState(""); setEditTaskComplement("");
    setEditTaskLatitude(""); setEditTaskLongitude("");
  };
    ...
    <div className="min-h-[100dvh] bg-[#F5F0E6] flex flex-col font-sans">
      ...
      <header className="bg-[#2C3E50] text-white px-4 py-3 shadow-md border-b border-[#2C3E50] z-20">
        <div className="flex justify-between items-center max-w-[1920px] mx-auto w-full">
          <div className="flex items-center gap-4">
            ...
            <div><div className="flex items-center gap-2"><Layout className="w-5 h-5 text-[#D35400]" /><h1 className="text-lg font-bold">Fluxo de Produto</h1></div></div>
          </div>
        ...
        <div className="flex gap-4 min-w-max h-full items-start">
          ...
          {tasks.filter(t => !t.columnId).length > 0 && (
             <div className="w-80 flex-shrink-0 flex flex-col max-h-[calc(100vh-140px)] rounded-xl bg-orange-50 border border-orange-100 shadow-sm" onDragOver={(e) => e.preventDefault()} onDrop={(e) => handleDrop(e, null)}>
                ...
                <div className="p-3 overflow-y-auto flex-1 space-y-3">{tasks.filter(t => !t.columnId).map(task => <TaskCard key={task.id} task={task} />)}</div>
             </div>
          )}
        </div>
      </main>
      ...
      <Dialog open={isTaskModal || isEditTaskModal} onOpenChange={(o) => { if (!o) { setIsTaskModal(false); setIsEditTaskModal(false); } }}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <form onSubmit={(e) => { e.preventDefault(); isEditTaskModal ? updateTask() : createTask(); }}>
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2">{isEditTaskModal ? <Edit className="w-5 h-5" /> : <Plus className="w-5 h-5 text-[#D35400]" />} {isEditTaskModal ? "Editar Tarefa" : "Nova Tarefa"}</DialogTitle>
            </DialogHeader>
            <div className="grid gap-6 py-4">
              ...
              <div className="space-y-4">
                <div className="grid gap-2">
                  ...
                  <Input value={isEditTaskModal ? editTaskTitle : taskTitle} onChange={(e) => isEditTaskModal ? setEditTaskTitle(e.target.value) : setTaskTitle(e.target.value)} placeholder="Ex: Refatorar Homepage" />
                </div>
                <div className="grid gap-2">
                  <Label>Descrição</Label>
                  <Textarea value={isEditTaskModal ? editTaskDescription : taskDescription} onChange={(e) => isEditTaskModal ? setEditTaskDescription(e.target.value) : setTaskDescription(e.target.value)} placeholder="Detalhes..." />
                </div>
              </div>
              ...
              <div className="bg-slate-50 p-4 rounded-lg border border-slate-100 space-y-3">
                ...
                <div className="grid grid-cols-[140px_1fr] gap-3">
                  <div className="relative">
                    <Input placeholder="CEP" maxLength={9} value={isEditTaskModal ? editTaskZip : taskZip} onChange={(e) => isEditTaskModal ? setEditTaskZip(e.target.value) : setTaskZip(e.target.value)} onBlur={() => handleCepSearch(isEditTaskModal)} />
                    {isSearchingCep && <div className="absolute right-2 top-2.5"><RefreshCw className="w-4 h-4 animate-spin text-slate-400" /></div>}
                  </div>
                ...
                <div className="grid grid-cols-[100px_1fr] gap-3">
                  <Input placeholder="Nº" value={isEditTaskModal ? editTaskNumber : taskNumber} onChange={(e) => isEditTaskModal ? setEditTaskNumber(e.target.value) : setTaskNumber(e.target.value)} />
                  <Input placeholder="Bairro" value={isEditTaskModal ? editTaskNeighborhood : taskNeighborhood} onChange={(e) => isEditTaskModal ? setEditTaskNeighborhood(e.target.value) : setTaskNeighborhood(e.target.value)} />
                </div>
                <div className="grid grid-cols-[1fr_80px] gap-3">
                  <Input placeholder="Cidade" value={isEditTaskModal ? editTaskCity : taskCity} onChange={(e) => isEditTaskModal ? setEditTaskCity(e.target.value) : setTaskCity(e.target.value)} />
                  <Input placeholder="UF" value={isEditTaskModal ? editTaskState : taskState} onChange={(e) => isEditTaskModal ? setEditTaskState(e.target.value) : setTaskState(e.target.value)} />
                </div>
                ...
                <div className="pt-2 border-t border-slate-200">
                    <div className="flex justify-between items-center mb-2">
                        <Label className="text-xs font-bold text-slate-500 uppercase tracking-wider flex items-center gap-1"><Globe className="w-3 h-3"/> Coordenadas</Label>
                        <Button type="button" variant="outline" size="sm" onClick={() => handleGeocode(isEditTaskModal)} disabled={isGeocoding} className="h-7 text-xs border-indigo-200 text-indigo-700 hover:bg-indigo-50">
                            {isGeocoding ? <RefreshCw className="w-3 h-3 animate-spin mr-1"/> : <Search className="w-3 h-3 mr-1"/>}
                            {isGeocoding ? "Buscando..." : "Buscar Coordenadas"}
                        </Button>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                        <Input placeholder="Latitude" value={isEditTaskModal ? editTaskLatitude : taskLatitude} onChange={(e) => isEditTaskModal ? setEditTaskLatitude(e.target.value) : setTaskLatitude(e.target.value)} type="number" step="any" className="bg-white" />
                        <Input placeholder="Longitude" value={isEditTaskModal ? editTaskLongitude : taskLongitude} onChange={(e) => isEditTaskModal ? setEditTaskLongitude(e.target.value) : setTaskLongitude(e.target.value)} type="number" step="any" className="bg-white" />
                    </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="grid gap-2">
                  ...
                  <select className="flex h-10 w-full rounded-md border bg-background px-3 py-2 text-sm" value={isEditTaskModal ? editTaskPriority : taskPriority} onChange={(e) => isEditTaskModal ? setEditTaskPriority(e.target.value) : setTaskPriority(e.target.value)}>
                    <option value="1">Alta</option><option value="2">Média</option><option value="3">Baixa</option>
                  </select>
                </div>
                <div className="grid gap-2">
                  <Label>Coluna</Label>
                  <select className="flex h-10 w-full rounded-md border bg-background px-3 py-2 text-sm" value={isEditTaskModal ? editTaskColumn : taskColumn} onChange={(e) => isEditTaskModal ? setEditTaskColumn(e.target.value) : setTaskColumn(e.target.value)}>
                ...
                <div className="grid gap-2">
                  ...
                  <Input type="datetime-local" value={isEditTaskModal ? editTaskScheduledDate : taskScheduledDate} onChange={(e) => isEditTaskModal ? setEditTaskScheduledDate(e.target.value) : setTaskScheduledDate(e.target.value)} />
                </div>
                <div className="grid gap-2">
                  <Label>Prazo Final (Due Date)</Label>
                  <Input type="datetime-local" value={isEditTaskModal ? editTaskDueDate : taskDueDate} onChange={(e) => isEditTaskModal ? setEditTaskDueDate(e.target.value) : setTaskDueDate(e.target.value)} />
                </div>
              </div>
              ...
              <div className="grid gap-2 mt-4">
                 ...
                 <select className="flex h-10 w-full rounded-md border bg-background px-3 py-2 text-sm" value={isEditTaskModal ? editTaskAssignedTo : taskAssignedTo} onChange={(e) => isEditTaskModal ? setEditTaskAssignedTo(e.target.value) : setTaskAssignedTo(e.target.value)}>
              ...
              <div className="grid gap-2">
                 ...
                 <Textarea className="min-h-[80px]" placeholder="Informações de conclusão..." value={isEditTaskModal ? editTaskFinalComment : taskFinalComment} onChange={(e) => isEditTaskModal ? setEditTaskFinalComment(e.target.value) : setTaskFinalComment(e.target.value)} />
              </div>
              ...
              <div className="border-t pt-4 space-y-4">
                 ...
                 <div className="flex gap-2">
                    <Label className="cursor-pointer border border-dashed p-3 rounded-md flex items-center gap-2 hover:bg-slate-50"><ImageIcon className="w-4 h-4" /> Add Imagem <Input type="file" accept="image/*" multiple className="hidden" onChange={(e) => { if (e.target.files) { const f = Array.from(e.target.files); isEditTaskModal ? setEditTaskImages(p => [...p, ...f]) : setTaskImages(p => [...p, ...f]); } }} /></Label>
                    <Label className="cursor-pointer border border-dashed p-3 rounded-md flex items-center gap-2 hover:bg-slate-50"><FileVideo className="w-4 h-4" /> Add Vídeo <Input type="file" accept="video/*" multiple className="hidden" onChange={(e) => { if (e.target.files) { const f = Array.from(e.target.files); isEditTaskModal ? setEditTaskVideos(p => [...p, ...f]) : setTaskVideos(p => [...p, ...f]); } }} /></Label>
                    <Button type="button" variant={isRecording ? "destructive" : "outline"} onClick={isRecording ? stopRecording : startRecording} className={isRecording ? "animate-pulse" : ""}><Mic className="w-4 h-4 mr-2" /> {isRecording ? "Gravando..." : "Gravar Áudio"}</Button>
                 </div>
              </div>
            </div>
            <DialogFooter><Button type="button" variant="outline" onClick={() => { setIsTaskModal(false); setIsEditTaskModal(false); }}>Cancelar</Button><Button type="submit" disabled={isSubmitting} className="bg-[#D35400] text-white">{isSubmitting ? "Salvando..." : "Salvar"}</Button></DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isColumnModal} onOpenChange={setIsColumnModal}>
        <DialogContent className="sm:max-w-[425px]">
          <form onSubmit={(e) => { e.preventDefault(); editingCol ? updateColumn() : createColumn(); }}>
            <DialogHeader><DialogTitle>{editingCol ? "Renomear" : "Nova Coluna"}</DialogTitle></DialogHeader>
            <div className="grid gap-4 py-4"><div className="grid gap-2"><Label>Título</Label><Input value={colTitle} onChange={(e) => setColTitle(e.target.value)} /></div></div>
            <DialogFooter><Button type="button" variant="outline" onClick={() => setIsColumnModal(false)}>Cancelar</Button><Button type="submit" className="bg-[#2C3E50] text-white">Salvar</Button></DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
            ...
            <div className="flex flex-col h-full bg-white">
              ...
              <div className="p-6 space-y-6">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-slate-50 rounded-lg border">
                   <div><span className="text-[10px] font-bold text-slate-400 uppercase">Responsável</span><div className="flex items-center gap-2 text-sm font-medium"><User className="w-3 h-3" /> {previewTask.assignedTo?.name || "-"}</div></div>
                ...
                <div className="border-t pt-4">
                   <h3 className="text-sm font-bold uppercase mb-4">Anexos ({previewTask.taskImages.length + previewTask.taskVideos.length + previewTask.taskAudios.length})</h3>
                   {previewTask.taskImages.length > 0 && <div className="grid grid-cols-4 gap-2 mb-4">{previewTask.taskImages.map(img => <img key={img.id} src={img.url} className="w-full aspect-square object-cover rounded border cursor-pointer" onClick={() => window.open(img.url, '_blank')} />)}</div>}
                   {previewTask.taskVideos.length > 0 && <div className="grid grid-cols-2 gap-2 mb-4">{previewTask.taskVideos.map(v => <video key={v.id} src={v.url} controls className="w-full rounded border" />)}</div>}
                   {previewTask.taskAudios.length > 0 && <div className="grid grid-cols-2 gap-2">{previewTask.taskAudios.map(a => <audio key={a.id} src={a.url} controls className="w-full h-10" />)}</div>}
                </div>
              </div>
