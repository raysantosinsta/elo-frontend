...
import { ArrowLeft, CheckCircle, Loader2, MapPin, AlertTriangle } from 'lucide-react';
...
export default function DriverPage() {
    ...
    // 3. Abre Modal com Lógica de Data Inteligente
    const handleOpenModal = (type: 'COMPLETED' | 'FAILED') => {
        ...
        if (type === 'COMPLETED') {
            // Sucesso: Sugere daqui a 30 dias (ex: retorno mensal)
            date.setDate(date.getDate() + 30);
        } else {
            // Problema: Sugere para AMANHÃ (ex: tentar entregar de novo)
            date.setDate(date.getDate() + 1);
        }

        setRescheduleDate(date.toISOString().split('T')[0]); // YYYY-MM-DD
        setIsModalOpen(true);
    };

    // 4. Envia para o Backend
    const confirmFinalization = async () => {
        // Validação: Problema exige comentário
        ...
        try {
            await api.patch(
                `/tasks/${task.id}/finalize`,
                {
                    status: actionType,
                    finalComment:
                        comment || (actionType === 'COMPLETED'
                            ? 'Entrega realizada'
                            : 'Não entregue'),
                    ...
                    <div className="bg-white w-full max-w-sm rounded-2xl p-6 animate-in slide-in-from-bottom-10 shadow-2xl">
                        ...
                        <div className="text-center mb-4">
                            <h3 className={`text-xl font-bold ${actionType === 'COMPLETED' ? 'text-emerald-600' : 'text-red-600'}`}>
                                {actionType === 'COMPLETED' ? 'Confirmar Entrega' : 'Reportar Problema'}
                            </h3>
                        ...
                        <div className="mb-4">
                            <label className="block text-sm font-medium text-slate-700 mb-1">
                                {actionType === 'COMPLETED' ? 'Próxima Visita' : 'Nova Tentativa'}
                            </label>
                            <input
                                type="date"
                                className={`w-full p-3 border rounded-xl text-sm outline-none 
                                    ${actionType === 'FAILED' ? 'focus:ring-red-500 bg-red-50 border-red-200' : 'focus:ring-emerald-500 bg-emerald-50 border-emerald-200'}
                                `}
                        ...
                        <div className="mb-4">
                            <label className="block text-sm font-medium text-slate-700 mb-1">
                                Observação {actionType === 'FAILED' && <span className="text-red-500">*</span>}
                            </label>
                            <textarea
                                value={comment}
