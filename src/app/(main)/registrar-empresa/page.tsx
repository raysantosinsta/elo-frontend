...
import { Building2, CheckCircle2, ChevronLeft, Loader2, MapPin, Search } from "lucide-react"
...
export default function CompanyRegistrationPage() {
  ...
  async function onSubmit(data: CompanyFormValues) {
    ...
    try {
      ...
      // --- 3. TRATAMENTO DE ERRO DO AXIOS ---
      let errorMessage = "Erro ao conectar com o servidor."
      
      if (error.response) {
        // Erro veio do backend (400, 401, 403, 500)
        const serverMessage = error.response.data?.message
        
        if (Array.isArray(serverMessage)) {
            errorMessage = serverMessage[0] // NestJS Class Validator retorna array
      ...
      <Card className="w-full max-w-4xl bg-white shadow-xl border border-[#95A5A6]/20 rounded-xl overflow-hidden">
        ...
        <CardHeader className="bg-white border-b border-[#F5F0E6] pb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle className="text-2xl font-bold text-[#2D3436]">Cadastrar Nova Empresa</CardTitle>
              <CardDescription className="text-[#95A5A6] mt-1">
                Preencha os dados institucionais para iniciar o fluxo de produção.
              </CardDescription>
            </div>
          </div>
        </CardHeader>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-[#2D3436]">Razão Social / Nome Fantasia</FormLabel>
                        <FormControl>
                          <Input 
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-[#2D3436]">E-mail Corporativo</FormLabel>
                        <FormControl>
                          <Input 
                            type="email" 
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem className="md:col-span-2">
                        <FormLabel className="text-[#2D3436]">Ramo de Atividade (Opcional)</FormLabel>
                        <FormControl>
                          <Input 
